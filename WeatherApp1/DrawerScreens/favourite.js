...
import { Button, View,Text,StyleSheet,ImageBackground, Image, TouchableOpacity,FlatList,Alert} from 'react-native';
...
function FavouritesScreen({ navigation }) {
  ...
  const dispatchDeleteAll = Items=>{
    Alert.alert('','Are you sure you want to remove all the favourites?',[
      {
        text: 'NO',
        onPress: () => null,
        style: 'cancel',
      },
      {text: 'YES', onPress: () => dispatch(deleteAllFav(Items))},
    ]);
  };
